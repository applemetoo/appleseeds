...
/*
	* d2 edits for magic finding sorc
	*
	*	randomizing the order of the areas that the sorc is mf in.
	*
*/

function LoadConfig() {
	/* Sequence config
	 * to change the order of the scripts, just change the order of their lines by using cut and paste.
	*/
	 
	// User addon script. Read the description in libs/bots/UserAddon.js
	Scripts.UserAddon = false; // must be set to false for boss/area scripts
	...
	/*
		*	Boss/area scripts
		*		utilize an array to randomize the areas being mf
		*
	*/
	
	var d2_acts = [1,2,3,4,5];
	d2_acts.sort(function(a,b){return 0.5 - Math.random()}); // randomizes acts 1 thru 5
	
	var i;
	for(i = 0; i < d2_acts.sort; i++) {
	...
	*/
	...
	Config.PickitFiles.push("pots_B.nip");
	...
	*/
	...
	Config.StaticList = ["Countess","Andariel","Radament","Summoner","Duriel","Mephisto","Izual","Hephasto","Diablo","Baal"]; // List of monster NAMES or CLASSIDS to static. Example: Config.StaticList = ["Andariel", 243];
